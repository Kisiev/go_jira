...
func (j JiraReportCommand) Run(update telegramEntity.TelegramUpdate) {
	...
	rawJiraFilter := fmt.Sprintf("project = TRACEWAY and worklogDate >= '%s' and worklogDate < '%s' and worklogAuthor = %s ORDER BY created DESC", dateStart, dateEnd, user.UserName)
	go getYesterdayTasks(rawJiraFilter, yesterdayChan)

	todayChan = make(chan entity.JiraTask)
	rawJiraFilter = fmt.Sprintf("project = TRACEWAY and assignee=%s and status not in (Закрыто, Выполнено, Done, CLOSED, Canceled) ORDER BY created DESC, priority DESC", user.UserName)
